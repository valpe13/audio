#!/usr/bin/env python3
"""Safe SVD-XT 1.1 checkpoint downloader for ComfyUI.
...
DEFAULT_URL = (
    ...
    "stable-video-diffusion-img2vid-xt-1-1/resolve/main/svd_xt_1_1.safetensors"
)
DEFAULT_FILENAME = "svd_xt.safetensors"
PLACEHOLDER_VALUES = {"", "PASTE_HUGGINGFACE_DIRECT_URL_HERE"}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Safely download the Stability AI SVD-XT 1.1 checkpoint for ComfyUI."
    )
    parser.add_argument(
        "--url",
        default=os.environ.get("SVD_XT_MODEL_URL", DEFAULT_URL),
        help=(
            "Direct SVD-XT model URL, preferably the Stability AI "
            "stable-video-diffusion-img2vid-xt-1-1 /resolve/main/*.safetensors "
            "URL. Can also be provided through SVD_XT_MODEL_URL."
        ),
    )
    parser.add_argument(
        "--target-dir",
...
def make_request(url: str) -> urllib.request.Request:
    headers = {"User-Agent": "XTTS-Studio-SVD-XT-Installer/1.0"}
...
def main() -> int:
    ...
    if url in PLACEHOLDER_VALUES:
        print("ERROR: No SVD-XT model URL is configured.", file=sys.stderr)
        print(
            "Set SVD_XT_MODEL_URL to a direct Stability AI SVD-XT "
            "/resolve/main/*.safetensors URL, or edit MODEL_URL in the .cmd file.",
            file=sys.stderr,
        )
    ...
    print("WARNING: SVD-XT 1.1 is a large image-to-video checkpoint.")
    print("The default URL points to Stability AI Stable Video Diffusion XT 1.1.")
    print("Hugging Face may require login and accepting model terms before download.")
    ...
    print(f"In XTTS Studio set video_i2v_model_checkpoint={filename}")
    return 0
