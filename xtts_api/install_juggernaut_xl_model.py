#!/usr/bin/env python3
"""Safe Juggernaut XL quality/balanced checkpoint downloader for ComfyUI.
...
DEFAULT_FILENAME = "juggernautXL.safetensors"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Safely download a Juggernaut XL SDXL quality/balanced checkpoint "
            "for ComfyUI."
        )
    )
    parser.add_argument(
        "--url",
        default=os.environ.get("JUGGERNAUT_XL_MODEL_URL", ""),
        help=(
            "Direct model URL, preferably a Hugging Face "
            "/resolve/main/*.safetensors URL. Can also be provided through "
            "JUGGERNAUT_XL_MODEL_URL."
        ),
    )
    parser.add_argument(
        "--target-dir",
...
def make_request(url: str) -> urllib.request.Request:
    headers = {"User-Agent": "XTTS-Studio-JuggernautXL-Installer/1.0"}
...
def main() -> int:
    ...
    if url in PLACEHOLDER_VALUES:
        print("ERROR: No Juggernaut XL model URL is configured.", file=sys.stderr)
        print(
            "Set JUGGERNAUT_XL_MODEL_URL to a direct Hugging Face "
    ...
    print("WARNING: Juggernaut XL is a large SDXL quality/balanced checkpoint.")
